function [x,f,funEvals] = minConf_TMP(funObj,x,LB,UB,options,ckpntFun)
...
[verbose,numDiff,optTol,maxIter,suffDec,interp,method,corrections,damped] = ...
    myProcessOptions(...
    options,'verbose',3,'numDiff',0,'optTol',1e-6,'maxIter',100,'suffDec',1e-4,...
    'interp',1,'method','lbfgs','corrections',10,'damped',0);
...
% Plot Progress
if (nargin == 6)
    [stop, dispstr] = ckpntFun(x);
    fprintf('\b    %s\n', dispstr);
    if stop, fprintf('Early termination condition(s) reached\n'); return; end
end
